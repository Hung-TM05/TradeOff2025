...
#---- plot: Biological rate hypothesis ----
pdf("./0. Grouped Data/Endo_Ecto/Endo_Ecto_Bio_rate_hypo_230923_Chelsa_dropNA.pdf", width=8, height=8, onefile = TRUE)
...
for(i in seq(1,2)){
  ...
  #---- 1. TTrange-CTmax ----
  print('Now processing panel 1')
  plot(x=df$CTmax, y=df$CTrange, las=1, cex= 3, 
       xlab = '', ylab = '',main = main_text,
       xlim = c(20,60), ylim = c(0,60),
       type="n", axes = F)
  box()
  # x-axis
  axis(side=1, at=seq(20,60,10),cex.axis=1.2)
  mtext(side=1, line=3, "Critical thermal maximum (°C)", col="black", font=1,cex=1.2)
  # y-axis
  axis(side=2, las=2, at=seq(0,60,10),cex.axis=1.2)
  mtext(side=2, line=3, "Thermal tolerance range (°C)", col="black", font=1,cex=1.2)
  ...
  if(p > 0.05) lty = 2 else lty = 1
  lines(x=mydata.x$df_x,y=pred.x$fit,col='black',lwd=1.5,lty=lty)
  points(x=df$CTmax,y=df$CTrange,col=scales::alpha(col.list[1], 0.7),pch=16,cex=0.7)
  
  #---- 2. TTrange-STmin ----
  print('Now processing panel 2')
  plot(x=df$CTmin, y=df$CTrange, las=1, cex= 3, 
       xlab = '', ylab = '',main = main_text,
       xlim = c(-20,40), ylim = c(0,60),
       type="n", axes = F)
  box()
  # x-axis
  axis(side=1, at=seq(-20,40,10),cex.axis=1.2)
  mtext(side=1, line=3, "Critical thermal minimum (°C)", col="black", font=1,cex=1.2)
  # y-axis
  axis(side=2, las=2, at=seq(0,60,10),cex.axis=1.2)
  mtext(side=2, line=3, "Thermal tolerance range (°C)", col="black", font=1,cex=1.2)
  ...
  if(p > 0.05) lty = 2 else lty = 1
  lines(x=mydata.x$df_x,y=pred.x$fit,col='black',lwd=1.5,lty=lty)
  points(x=df$CTmin,y=df$CTrange,col=scales::alpha(col.list[3], 0.7),pch=16,cex=0.7)
...
df = all_data
df_x = df$TTRS_DTR
df_y = df$TTRS_CTrange
md <- lme(df_y~ df_x+ TTRS_Weight, random= ~1|Family, data = df, na.action=na.exclude)
sum = summary(md)
sum
# p-value still < 0.05, don't need to change.